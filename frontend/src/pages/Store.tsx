...
const STORE_ITEMS = [
    ...
    },
    {
        ...
        priceType: 'usd',
        rarity: 'Popular',
        rarityColor: 'bg-tertiary-fixed-dim text-[#001f27]',
...
const CATEGORIES = ['Featured', 'Coin Packs', 'Exotic Tiles', 'Emotes'];
...
export default function Store() {
    ...
    const claimSkin = (skinId: string | null) => {
        if (!skinId) return;
                ...
                <header className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
                    ...
                    <div className="flex items-center gap-4 bg-surface-container-high px-6 py-4 rounded-2xl border border-outline-variant/10">
                        <div className="flex flex-col">
                            ...
                            <span className="text-2xl font-headline font-bold text-primary">{user?.coins?.toLocaleString() || '0'} COINS</span>
                        </div>
                        ...
                        return (
                            <div key={item.id} className="glass-card rounded-[2rem] p-6 flex flex-col group hover:bg-surface-container-highest/60 transition-all duration-500 border border-outline-variant/5">
                                <div className="relative w-full aspect-square mb-6 rounded-2xl overflow-hidden bg-surface-container-lowest flex items-center justify-center">
                                    ...
                                    {item.rarity && (
                                        <div className={`absolute top-4 left-4 px-3 py-1 ${item.rarityColor} text-[10px] font-bold rounded-full uppercase tracking-tighter`}>{item.rarity}</div>
                                    )}
                                </div>
                                ...
                                <div className="flex items-center justify-between gap-4">
                                    <div>
                                        {item.priceType === 'coins' ? (
                                            <div className="flex items-center gap-1">
                                                ...
                                                <span className="font-bold text-lg">{(item.price as number).toLocaleString()}</span>
                                            </div>
                                        ) : (
                                            <div className="flex flex-col">
                                                {item.originalPrice && <span className="text-[10px] uppercase text-on-surface-variant line-through">${item.originalPrice}</span>}
                                    ...
                                    {owned ? (
                                        ...
                                        <button
                                            onClick={() => claimSkin(item.skinId)}
                                            className="flex-1 bg-gradient-to-r from-primary-container to-primary py-3 rounded-xl font-bold text-on-primary text-sm shadow-lg shadow-primary/10 active:scale-95 transition-all"
                                        >
                                            Buy
                                        </button>
                                    )}
                ...
                <section className="mt-20 relative overflow-hidden rounded-[2.5rem] bg-surface-container h-80 flex items-center border border-outline-variant/10">
                    ...
                    <div className="relative z-10 pl-12 pr-6 max-w-2xl">
                        ...
                        <div className="flex gap-4">
                            <button className="bg-gradient-to-r from-primary-container to-primary px-8 py-3 rounded-xl font-bold text-on-primary shadow-xl shadow-primary/20 hover:-translate-y-1 transition-all">Claim Now</button>
